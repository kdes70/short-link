    ...
    <div class="p-6 max-w-sm w-full bg-white shadow-md rounded-md">
      ...
      <form class="mt-4" @submit.prevent="handleLogin">
        ...
        <div class="mt-6">
          ...
          <div class="mt-6">
            <router-link class="block text-sm fontme text-indigo-700 hover:underline"
                         :to="{name: 'Register'}">Don't have an account?
            </router-link>

            <div v-if="error" class="mt-3">
              <span v-text="error" class="text-red-500"></span>
            </div>
  ...
  methods: {
    handleLogin() {
      if (this.email && this.password) {
        ...
        this.$store.dispatch(AUTH_LOGIN, [this.email, this.password])
            .then(res => {
              if (this.isAuth) {
